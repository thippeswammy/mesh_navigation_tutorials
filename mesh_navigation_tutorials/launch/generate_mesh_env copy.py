...
def main():
    ...
    parser.add_argument("--boolean-union", action="store_true", help="Attempt to fuse all meshes using boolean union (requires manifold meshes, computationally intensive)")
    ...
    # 1. Initial Merge with Welding (or Boolean Union)
    if args.boolean_union:
        print("Attempting Boolean Union of all sub-meshes...")
        try:
            # Filter for watertight meshes only
            valid_meshes = []
            for d in mesh_data_list:
                m = d['mesh']
                if m.is_watertight:
                    valid_meshes.append(m)
                else:
                    print(f"  Warning: Mesh from {d.get('source_path')} is not watertight. attempting repair...")
                    trimesh.repair.fill_holes(m)
                    if m.is_watertight:
                        valid_meshes.append(m)
                    else:
                        print("  Failed to repair. Skipping from boolean union (will just concatenate).")
            
            if len(valid_meshes) > 1:
                # Use engines if available (blender, manifold)
                # 'scad' is slow, 'blender' is good. trimesh tries available ones.
                initial_merged_mesh = trimesh.boolean.union(valid_meshes)
                print("  Boolean Union successful!")
                
                # Check result
                if isinstance(initial_merged_mesh, trimesh.Scene):
                     initial_merged_mesh = trimesh.util.concatenate(initial_merged_mesh.geometry.values())

            else:
                 print("  Not enough watertight meshes for union. Fallback to concatenation.")
                 initial_merged_mesh = trimesh.util.concatenate([d['mesh'] for d in mesh_data_list])

        except Exception as e:
            print(f"  Boolean Union failed: {e}. Fallback to concatenation.")
            initial_merged_mesh = trimesh.util.concatenate([d['mesh'] for d in mesh_data_list])
    else:
